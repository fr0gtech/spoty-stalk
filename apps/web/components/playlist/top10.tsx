...
function Top10(props: any) {
  ...
  const { data: artists, error: pl_error } = useSWR(`/api/artists`, fetcher);
  const createLink = (artist: any) => {
    if (openInApp && artist.platform === "spotify")
      return `spotify:artist:${artist.sid}`;
  ...
  return (
    <div className="mx-auto flex gap-5 text-xs items-center h-fit flex-wrap">
      ...
      {artists &&
        artists.data.map((artist: any, i: any) => {
          return (
                ...
                <Tooltip2
                  ...
                  content={
                    <div className="text-xs">
                      <div>{artist.name}</div>songs: {artist._count.songs}
                    </div>
                  }
                >
