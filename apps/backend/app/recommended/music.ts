...
export const findMusicPosts = (e: any) => {    
    if (e.media.oembed.provider_url.includes('soundcloud')) {
        const url = htmlDecode(e.media.oembed.html)
            ...
            const parsedAPIurl = querystring.parse(parsedUrl[Object.keys(parsedUrl)[0]] as string)
    ...
    if (e.media.oembed.provider_url.includes("spotify")) {
        ...
        if (new URL(parsedAPIurl).pathname.split('/')[2] === "track"){
            type = "song"
            artist = e.media.oembed.description.split('·')[0].split('on Spotify.')[1].trim()
            ...
            type = "album"
            artist = unescapeHtml(e.media.oembed.description.split('Spotify.')[1].split('·')[0].trim())
            ...
            type = "artist"
            artist = e.media.oembed.description.split('·')[0].split('on Spotify.')[0].replace('Listen to', '').trim()
        }else{
...
export const uploadAll = async (all: RecommendedItem[]) => {
    ...
    all.forEach(async (e)=>{
        await prisma.recommended.createMany({
                data: {
                    ...e
                }
            }).catch((err) => {
