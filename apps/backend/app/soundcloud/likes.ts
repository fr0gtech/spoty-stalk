...
export const getLikesTo = async (getTo: any) => {
  ...
  while (next !== null) {
    if (next) {
      ...
      await axios.get(nextUrl.toString()).then((e: any) => {
        const a = e.data.collection.find(
          (e: any) => e.track.id === parseInt(getTo.sid)
        );
        if (a) {
      ...
      await axios
        .get(
          `https://api-v2.soundcloud.com/users/${userId}/likes?client_id=${client_id}`
        )
        .then((e: any) => {
          const a = e.data.collection.find(
            (e: any) => e.track.id === parseInt(getTo.sid)
  ...
  const index = allLikes.findIndex(
    (object: any) => object.track.id === parseInt(getTo.sid)
  );
